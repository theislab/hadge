...
if (args$findVariants == 'True' | args$findVariants == 'default'){
  ...
  for (donorid in donors){
    ...
    write.csv(matched_gt, file.path(outputdir_variant, paste0(donorid, "_matched_gt.csv")), row.names = F)
    write.csv(unmatched_gt_list, file.path(outputdir_variant, paste0(donorid, "_unmatched_gt.csv")), row.names = F)
    ...
    write.table(unique(informative_variants_cells$variant), file.path(outputdir_variant, paste0(donorid, "_informative_variants.csv")),
  ...
  upset <- ComplexUpset::upset(representative_variant_all, donors, width_ratio = 0.45, height_ratio = 0.9,
                               stripes = 'white', max_degree=1,
                               name="Number of donor-specific variants",
                               set_sizes=(upset_set_size() + 
                                            geom_text(aes(label = ..count.., size=3), hjust = -0.1, 
                                                      stat = 'count', color = 'white', size = 2.3) +
  ...
  write.table(representative_variant_single[,c("chr", "pos")], quote = F, col.names=F,sep='\t',
              file.path(args$outputdir, "donor_specific_variants.csv"), row.names = F)
}
