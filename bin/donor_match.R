...
convert2binary <- function(result_csv, method_name, min_cell) {
  ...
  if (length(unique(method_assign[[method_name]])) == 1) {
    method_assign_binary <- as.data.frame(matrix(0, nrow = nrow(result_csv), ncol = 1),
      row.names = result_csv$Barcode
    )
    colnames(method_assign_binary) <- c(unique(method_assign[[method_name]]))
    method_assign_binary[rownames(method_assign_binary) %in% method_assign$Barcode, ] <- 1
  } else {
...
hashing_methods <- c("demuxem", "htodemux", "multiseq", "hashsolo", "hashedDrops", "bff", "gmm_demux")
...
for (i in 1:length(method1_all)) {
  ...
  method1_res <- method1_res[rownames(method1_res) %in% intersect_barcode, , drop = FALSE]
  method2_res <- method2_res[rownames(method2_res) %in% intersect_barcode, , drop = FALSE]
  ...
  if (!all(is.na(correlation_res))) {
    ...
    correlation_res <- correlation_res[!is.na(row.names(correlation_res)), , drop = FALSE]
    correlation_res <- correlation_res[order(as.numeric(row.names(correlation_res))), , drop = FALSE]
    pheatmap(correlation_res,
...
if (args$findVariants == "True" || args$findVariants == "vireo") {
  ...
  representative_variant <- separate(representative_variant,
    col = "variants",
    into = c("chr", "pos"),
    sep = "_", extra = "drop"
  )
