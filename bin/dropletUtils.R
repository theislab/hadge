...
parser$add_argument("--confidentNmads",
  default = 3, type = "double",
...
emptyDrops_out <- emptyDrops(hto,
  lower = args$lower, niters = args$niters,
...
Argument <- c(
  "raw_hto_matrix_dir", "lower", "niters",
...
Value <- c(
  args$raw_hto_matrix_dir, args$lower, args$niters,
...
if (sum(is.na(hashedDrops_out$LogFC2)) != length(hashedDrops_out$LogFC2)) {
  ...
  plot(hashedDrops_out$LogFC, hashedDrops_out$LogFC2,
    col = colors,
