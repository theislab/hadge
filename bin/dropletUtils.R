...
parser$add_argument("--lower", default = 100, type = "double",
...
parser$add_argument("--byRank", default = NULL, type = "integer",
...
parser$add_argument("--isCellFDR", default = 0.01, type = "double",
                    help = "Threshold to filter the cells.")
parser$add_argument("--objectOutEmptyDrops", default = "emptyDroplets",
...
parser$add_argument("--pseudoCount", default = 5, type = "double",
                    help = "A numeric scalar specifying the minimum pseudo-count when computing logfold changes.")
parser$add_argument("--constantAmbient", action = "store_true",
...
parser$add_argument("--objectOutHashedDrops", default = "hashedDrops",
...
Argument <- c("raw_hto_matrix_dir", "lower", "niters",
              "testAmbient", "ignore", "alpha", "round",
              "byRank", "isCellFDR", "ambient", "minProp",
              "pseudoCount", "constantAmbient", "doubletNmads", "doubletMin",
              "doubletMixture", "confidentNmads", "confidenMin", "combinations")
Value <- c(args$raw_hto_matrix_dir, args$lower, args$niters,
           args$testAmbient, ignore, alpha, args$round,
           byRank, args$isCellFDR, args$ambient, minProp,
           args$pseudoCount, args$constantAmbient, args$doubletNmads,
           args$doubletMin, args$doubletMixture, args$confidentNmads,
           args$confidenMin, combinations)
...
write.csv(hashedDrops_out,
          paste0(args$outputdir, "/", args$assignmentOutHashedDrops, "_res.csv"))
saveRDS(hashedDrops_out,
        file = paste0(args$outputdir, "/", args$objectOutHashedDrops, ".rds"))

colors <- ifelse(hashedDrops_out$Confident, "black",
